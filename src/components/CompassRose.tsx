...
export const CompassRose: React.FC = () => {
  ...
  const currentHeading = isHeadingLocked ? heading?.heading : 0;
    ...
    <div className="absolute right-4 bottom-4 pointer-events-none z-[900]">
      <div className="w-48 h-48">
        <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-lg">
          ...
          {/* Cardinal directions */}
          <g fill="white" fontSize="14" textAnchor="middle" dominantBaseline="middle" fontWeight="bold">
      ...
      <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 bg-black/70 px-3 py-1 rounded-lg text-white text-center backdrop-blur-sm whitespace-nowrap">
        {currentHeading !== undefined ? (
          <>
            <div className="text-lg font-bold">{currentHeading.toFixed(1)}°</div>
            <div className="text-xs opacity-75">
              {isHeadingLocked ? "Locked" : "Current"} Heading
            </div>
          </>
        ) : (
          <div className="text-sm opacity-75">No Heading Available</div>
        )}
      </div>
    </div>
  );
